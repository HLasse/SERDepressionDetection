...
# 
# wdf_full %>% 
#   group_by(visit, Diagnosis_only, Gender) %>% 
#   summarise(n = n(), 
#             Hamilton = mean(HamD17_Before, na.rm=TRUE), 
#             Hamilton_IQR = IQR(HamD17_Before, na.rm=TRUE), 
#             age = mean(Age, na.rm=T), 
#             Age_min = min(Age, na.rm=T), 
#             Age_max = max(Age, na.rm=T)) %>% 
#     mutate(age_range = paste(Age_min, Age_max, sep="-")) %>% 
#     ungroup() %>% 
#     select(-c(Age_min, Age_max, visit)) %>%
#     kbl(digits=1, col.names = c("Diagnosis", "Gender", "N",
#                               "Hamilton mean", "Hamilton IQR",
#                               "Age mean", "Age Range")) %>% 
#     kable_classic(full_width = F) %>% 
#     pack_rows("Visit 1", 1, 4) %>% 
#     pack_rows("Visit 2", 5, 8)
```
...
fig1_c <- samps_raw_mix %>% 
  preprocess_samples(.) %>% 
   mutate(key = case_when(
    as.character(key) == "Depression" ~ "Depression\n(first visit)\nN = 40",
    as.character(key) == "Remission" ~ "Depression\n(second visit)\nN = 25",
    as.character(key) == "Controls1" ~ "Controls\n(First visit)\nN = 42",
    as.character(key) == "Controls2" ~ "Controls\n(second visit)\nN = 25"
  )
  ) %>% 
...
fig1_c <- fig1_c + guides(fill=guide_legend(ncol=2), color=guide_legend(ncol=2))
fig1_a + fig1_b + fig1_c + guide_area() + plot_layout(guides = "collect")
```
...
make_roc_plot <- function(df, clean){
  ...
  thres_df <- pROC::coords(roc_30, 'best', transpose = F, ret=c("threshold", "specificity", "sensitivity", "precision")) %>% 
    bind_rows(pROC::coords(roc_full, "best", transpose = F, ret=c("threshold", "specificity", "sensitivity", "precision"))) %>% 
    mutate(AUC = c(auc_30, auc_full),
           across(where(is.numeric), round, 2),
           Type = c("Windows", "Summary")) %>% 
    relocate(Type, AUC) %>% 
    rename(Threshold = threshold, Specificity = specificity, Sensitivity = sensitivity, Precision = precision)
...
# how to correctly calculate the standard deviation of the two distributions?

# mu1_intercept
mu1 <- inv_logit_scaled(-1.18)
# mu2_intercept
mu2 <- inv_logit_scaled(0.38)

# sigma 1
sigma1_low <- inv_logit_scaled(-1.18 - 0.66)
sigma1_high <- inv_logit_scaled(-1.18 + 0.66)
# sigma 2
sigma2_low <- inv_logit_scaled(0.38 - 0.32)
sigma2_high <- inv_logit_scaled(0.38 + 0.32)

mu1
mu1 - sigma1_low
sigma1_high - mu1
# is this correct?
# which one to report? other way to calculate sd in probability space?
mu2
mu2 - sigma2_low
sigma2_high - mu2
...
tidy(mix_m, parameters = "^sigma") %>% 
  mutate(across(where(is.numeric), ~ inv_logit_scaled(.x)))
# Seems way too large when looking at the distributions. ~ 0.1 more likely. 
```
...
fig_s1_b <- (dep_heat + theme(legend.position = 'none')) / cont_heat  & theme(strip.background = element_rect(color = 'grey'))
```
...
BBB
"
fig_s1_a / fig_s1_b + plot_layout(design = layout) + plot_annotation(tag_levels = list(c("A", "B", "")))
```
...
brms_files <- c('best_brms', 'raw_brms', 'non_seg_brms')
...
main_post_plot / cohen_d_plot +
  plot_layout(guides = "collect", heights = unit(c(5, 0.5), c('cm'))) & 
  theme(legend.position = 'bottom')
```
...
### Figure S4 combined
```{r fig_s4}
fig_s4a$plot / fig_s4b + plot_annotation(tag_levels = "A")
```
